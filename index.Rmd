...
```{R}
library(readxl)
provinceNight <- read_excel("data/2000~2020年全国各省份夜间灯光数据.xlsx")
library(ggplot2)

gg <- ggplot(provinceNight) +
 geom_line(aes(年份,均值,color = 省份)) +
...
read_sf('data/2019行政区划/省.shp') -> provmap
read_sf('data/2019行政区划/市.shp') -> citymap
read_sf('data/2019行政区划/县.shp') -> countymap
read_sf("data/Coastline/海岸线.shp") -> hax
library(sf)
library(dplyr)
readxl::read_xlsx("data/2000~2020年全国各省份夜间灯光数据.xlsx") %>% 
  dplyr::filter(年份 == 2020) %>% 
  dplyr::select(-年份) %>% 
  left_join(provmap) %>% 
  st_sf() -> df3
...
df3 %>% 
  ...
  scico::scale_fill_scico(
    ...
    labels = c("昏暗", latex2exp::TeX("$\\leftarrow$ 夜间灯光亮度  $\\rightarrow$"), "Brightness"),
    name = "2020 年中国各城市\n平均夜间灯光亮度",
